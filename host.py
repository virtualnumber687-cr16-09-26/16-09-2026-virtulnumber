...
def main_menu_keyboard(user_id):
    ...
    kb.add(types.KeyboardButton("💸 Refund"))
    if is_admin(user_id):
...
# ---------------------------------------------------------------------------
# 💸 REFUND (অটো OTP-চেক সহ)
# ইউজার Refund বাটনে চাপে -> নম্বর দেয় -> OTP লিংক দেয় -> বট নিজে লিংক খুলে দেখে OTP
# এসেছে কিনা।  OTP এসেছে  -> Refund নেই
#               OTP আসেনি   -> অটো Refund (ব্যালেন্সে ফেরত)
#               চেক করা যায়নি / ফরম্যাট বোঝা যায়নি -> Refund হবে না, এডমিনকে জানানো হবে
# ---------------------------------------------------------------------------
REFUND_HTTP_TIMEOUT = 12   # সেকেন্ড
REFUND_MIN_WAIT_HOURS = 4  # কেনার পর কমপক্ষে এত ঘণ্টা পার না হলে Refund দেওয়া হবে না


def _hours_since_purchase(order):
    """অর্ডারের 'date' ফিল্ড থেকে কেনার পর কত ঘণ্টা পার হয়েছে তা রিটার্ন করে।
    কোনো কারণে পার্স করা না গেলে বড় একটা সংখ্যা রিটার্ন করে, যাতে ভুলভাবে
    সবার Refund ব্লক না হয়ে যায়।"""
    try:
        purchased_at = datetime.datetime.strptime(order.get("date", ""), "%Y-%m-%d %H:%M:%S")
        return (datetime.datetime.now() - purchased_at).total_seconds() / 3600
    except Exception:
        return REFUND_MIN_WAIT_HOURS + 1

# ⚙️ OTP চেকারের টিউনিং — OTP লিংকের রেসপন্স ফরম্যাট অনুযায়ী এখানে বদলাতে পারবেন
_OTP_CODE_RE = re.compile(r"(?<![\d])(?:\d{3}[-\s]\d{3}|\d{4,8})(?![\d])")
_NO_OTP_HINTS = (
    "no sms", "no message", "no otp", "no code", "not received", "no data",
    "waiting", "empty", "pending", "নেই", "আসেনি",
)
_YES_OTP_HINTS = ("code received", "otp received", "sms received")
...
def _flatten_json(obj, out):
    """JSON এর সব value টেক্সট হিসেবে out লিস্টে জমা করে; ফাঁকা list/dict/null হলে
    (অর্থাৎ কোনো মেসেজ নেই) True রিটার্ন করে।"""
    saw_empty = False
    if isinstance(obj, dict):
        if not obj:
            return True
        for v in obj.values():
            saw_empty |= _flatten_json(v, out)
...
def _analyze_otp_text(raw_text, content_type="", number=""):
    """রেসপন্স টেক্সট দেখে ('arrived' | 'not_arrived' | 'unknown', detail) রিটার্ন করে।"""
    text = raw_text or ""
    empty_marker = False
    try:
        if "json" in content_type.lower() or text.lstrip()[:1] in "{[":
            parts = []
            empty_marker = _flatten_json(json.loads(text), parts)
            text = " | ".join(parts)
    except Exception:
        pass
    ...
    text = re.sub(r"<[^>]+>", " ", text)
    # false-positive এড়াতে: URL, তারিখ, সময়, নিজের নম্বর বাদ
    text = re.sub(r"https?://\S+", " ", text)
    text = re.sub(r"\d{4}-\d{1,2}-\d{1,2}", " ", text)
    text = re.sub(r"\d{1,2}:\d{2}(?::\d{2})?", " ", text)
    num_digits = _digits(number)
    if num_digits:
        # নম্বরটা পেজে +/স্পেস/ড্যাশ যেভাবেই লেখা থাকুক (+12816684395, 1281-668-4395 ইত্যাদি),
        # সেটা বাদ দিয়ে দেয় যাতে নম্বরটাকে ভুলে OTP কোড ভেবে না বসে
        num_pattern = re.compile(r"\+?[\s-]*" + r"[\s-]*".join(list(num_digits)))
        text = num_pattern.sub(" ", text)
    text = re.sub(r"\s+", " ", text).strip()

    m = _OTP_CODE_RE.search(text)
    if m:
        return "arrived", m.group(0)
    low_early = text.lower()
    if any(h in low_early for h in _YES_OTP_HINTS):
        return "arrived", "code-received marker found (code text unparsed)"
    if not text:
        # JSON এ স্পষ্টভাবে ফাঁকা list/dict হলেই "OTP আসেনি"; একদম ফাঁকা বডি হলে নিশ্চিত নই -> unknown
        return ("not_arrived", "empty json") if empty_marker else ("unknown", "empty response body")
    low = text.lower()
    if empty_marker or any(h in low for h in _NO_OTP_HINTS):
        return "not_arrived", "no otp in response"
    return "unknown", "unrecognized response format"


def check_otp_arrived(link, number=""):
    """OTP লিংক খুলে ('arrived' | 'not_arrived' | 'unknown', detail) রিটার্ন করে।
    নেটওয়ার্ক/HTTP এরর হলে সবসময় 'unknown' (কখনোই অটো refund নয়)।"""
    import requests
    if not str(link).startswith(("http://", "https://")):
        return "unknown", "no valid link"
    try:
        r = requests.get(link, timeout=REFUND_HTTP_TIMEOUT, headers={"User-Agent": "Mozilla/5.0"})
    except Exception as e:
        return "unknown", f"request failed: {e}"
    if r.status_code != 200:
        return "unknown", f"http {r.status_code}"
    return _analyze_otp_text(r.text, r.headers.get("Content-Type", ""), _digits(number))


def _order_items(order):
    return order.get("items") or [{"number": order.get("number", ""), "otp_link": order.get("otp_link", "N/A")}]


def _find_purchased_item(uid, number_text):
    """ইউজারের নিজের কেনা অর্ডারগুলোর মধ্যে এই নম্বরটা খোঁজে -> (order, item) | (None, None)"""
    target = _digits(number_text)
    if len(target) < 6:
        return None, None
    for o in _user_orders(uid):
        for it in _order_items(o):
            if _digits(it.get("number")) == target:
                return o, it
    return None, None


def _is_refunded(order, number):
    return _digits(number) in [_digits(x) for x in order.get("refunded_numbers", [])]
...
@bot.message_handler(func=lambda m: m.text == "💸 Refund")
def menu_refund(message):
    if maintenance_block(message):
        return
    if require_force_join(message):
        return
    uid = message.from_user.id
    user_state[uid] = {"menu": "refund"}
    bot.send_message(
        message.chat.id,
        "💸 <b>Refund</b>\n\nযে নম্বরটির Refund চান সেই নম্বরটি দিন:",
        reply_markup=back_to_main_keyboard(),
    )
    bot.register_next_step_handler(message, process_refund_number)


def process_refund_number(message):
    uid = message.from_user.id
    text = (message.text or "").strip()
    if text in ["⬅️ Back", "⬅️ Back to Menu"]:
        go_back(message)
    ...
    order, item = _find_purchased_item(uid, text)
    if not order:
        bot.send_message(message.chat.id, "❌ এই নম্বরটি আপনার কেনা নম্বরের মধ্যে পাওয়া যায়নি। সঠিক নম্বর দিন:")
        bot.register_next_step_handler(message, process_refund_number)
        return

    if _is_refunded(order, item["number"]):
        bot.send_message(message.chat.id, "⚠️ এই নম্বরের Refund আগেই দেওয়া হয়েছে।", reply_markup=main_menu_keyboard(uid))
        return

    # ⏳ কেনার পর কমপক্ষে REFUND_MIN_WAIT_HOURS ঘণ্টা পার হতে হবে — সাথে সাথেই
    # Refund চাইলে (OTP আসার সুযোগ পাওয়ার আগেই) ব্লক করে দেওয়া হয়।
    hours_passed = _hours_since_purchase(order)
    if hours_passed < REFUND_MIN_WAIT_HOURS:
        remaining_minutes = int(round((REFUND_MIN_WAIT_HOURS - hours_passed) * 60))
        h, m = divmod(max(remaining_minutes, 1), 60)
        wait_text = f"{h} ঘণ্টা {m} মিনিট" if h else f"{m} মিনিট"
        bot.send_message(
            message.chat.id,
            f"⏳ <b>এখনো Refund দেওয়া যাবে না</b>\n\n"
            "আগে নাম্বারটি ব্যবহার করেন।\n"
            f"এই নম্বরটি কেনার পর কমপক্ষে {REFUND_MIN_WAIT_HOURS} ঘণ্টা পার হতে হবে।\n"
            f"আরও অপেক্ষা করুন: <b>{wait_text}</b>",
            reply_markup=main_menu_keyboard(uid),
        )
        return

    user_state[uid] = {"menu": "refund", "refund_order": order["order_id"], "refund_number": item["number"]}
    bot.send_message(message.chat.id, "🔗 এবার এই নম্বরের OTP লিংকটি দিন:", reply_markup=back_to_main_keyboard())
    bot.register_next_step_handler(message, process_refund_link)


def process_refund_link(message):
    uid = message.from_user.id
    text = (message.text or "").strip()
    if text in ["⬅️ Back", "⬅️ Back to Menu"]:
        go_back(message)
    ...
    state = user_state.get(uid, {})
    order = orders.get(state.get("refund_order"))
    number = state.get("refund_number")
    if not order or not number:
        bot.send_message(message.chat.id, "⚠️ আবার Refund বাটন থেকে শুরু করুন।", reply_markup=main_menu_keyboard(uid))
        return

    item = next((it for it in _order_items(order) if _digits(it.get("number")) == _digits(number)), None)
    stored_link = item.get("otp_link") if item else None

    # শুধু এই নম্বরের আসল (স্টকে থাকা) লিংকই গ্রহণযোগ্য — অন্য/ফেক লিংক দিয়ে ফাঁকি দেওয়া ঠেকাতে
    if stored_link and str(stored_link).startswith(("http://", "https://")) and _norm_link(text) != _norm_link(stored_link):
        bot.send_message(message.chat.id, "❌ এই লিংকটি এই নম্বরের নয়। সঠিক OTP লিংক দিন:")
        bot.register_next_step_handler(message, process_refund_link)
        return

    wait = bot.send_message(message.chat.id, "⏳ OTP এসেছে কিনা চেক করা হচ্ছে...")
    status, detail = check_otp_arrived(stored_link, number)
    ...
    if status == "not_arrived":
        with _refund_lock:
            ...
            u["balance"] += amount
            u["total_purchased"] = max(0, u.get("total_purchased", 0) - 1)
            order.setdefault("refunded_numbers", []).append(number)
            save_db()
        bot.send_message(
            message.chat.id,
            "✅ <b>Refund সফল!</b>\n\n"
            f"📞 নম্বর: <code>{number}</code>\n"
            f"💰 ফেরত: <code>{fmt_amount(amount)}</code>\n"
            f"💳 বর্তমান ব্যালেন্স: <code>{fmt_amount(u['balance'])}</code>",
            reply_markup=main_menu_keyboard(uid),
        )
        _notify_admins(
            "💸 <b>Auto Refund</b>\n"
    ...
    # unknown -> অটো refund নয়, এডমিন রিভিউ
    bot.send_message(
        message.chat.id,
        "⚠️ এখন OTP চেক করা সম্ভব হয়নি। আপনার Refund রিকোয়েস্ট এডমিনকে পাঠানো হয়েছে, একটু অপেক্ষা করুন।",
        reply_markup=main_menu_keyboard(uid),
    )
    _notify_admins(
        "⚠️ <b>Refund — ম্যানুয়াল রিভিউ দরকার</b>\n"
        f"👤 User: <code>{uid}</code>\n🆔 Order: <code>{order['order_id']}</code>\n"
        f"📞 Number: <code>{number}</code>\n🔗 Link: {stored_link}\n❓ কারণ: {detail}"
    )
    user_state[uid] = {"menu": "main"}
